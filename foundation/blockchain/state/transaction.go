...
import "github.com/DMV-Petri-Dish/crypto/foundation/blockchain/database"
...
func (s *State) UpsertWalletTransaction(signedTx database.SignedTx) error {
	...
	// Just check the signed transaction has a proper signature and
	// valid account for the recipient. It's up to the wallet to make sure
	// the account has proper balance and nonce. Fees will be taken if this
	// transaction is mined into a block and those types of validations fail.
...
func (s *State) UpsertNodeTransaction(tx database.BlockTx) error {
	...
	// Just check the signed transaction has a proper signature and valid account for the recipient
