...
import (
	...
	"github.com/DMV-Petri-Dish/crypto/foundation/blockchain/mempool/selector"
)
...
func (mp *Mempool) Upsert(tx database.BlockTx) error {
	...
	// consumed and some may limit based onthe number of transactions.
	// If a limit is met, then either the transaction that has the least return on investment
	// or the oldest will be dropped from the pool to make room.

	// For now, impose no limits
	key, err := mapKey(tx)
	if err != nil {
		return err
	}

	// Ethereum requires a 10% bump in the tip to replace an existing transaction
	// in the mempool and so do we. We want to limit users form this sort of behavior
