...
func (w *Worker) runPeersOperation() {
	...
	for _, peer := range w.state.RetrieveKnownPeers() {
		...
		// Add peers from this node's peer list that we are missing.
...
// addNewPeers takes the list of know peers and makes sure they are included
// in the nodes list of known peers
func (w *Worker) addNewPeers(knownPeers []peer.Peer) error {
	...
	for _, peer := range knownPeers {
		...
		if w.state.AddKnownPeer(peer) {
			w.evHandler("worker: runPerUpdatesOperation: addNewPeers: add peer nodes: adding peer-node %s", peer.Host)
		}
	}
